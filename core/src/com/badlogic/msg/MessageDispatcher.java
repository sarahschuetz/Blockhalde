...
public class MessageDispatcher implements Telegraph {
	...
	private Logger logger = new Logger("MessageDispatcher", Application.LOG_INFO);
	...
	private void discharge (Telegram telegram) {
		if (telegram.receiver != null) {
			...
			if (!telegram.receiver.handleMessage(telegram)) {
				...
				if (debugEnabled) logger.info("Message " + telegram.message + " not handled");
			}
		} else {
			...
			// Telegram could not be handled
			if (debugEnabled && handledCount == 0) logger.info( "Message " + telegram.message + " not handled");
		}
